...
We do not verify the emitted code, but its behavior.")
  ...
  (ensure-serialization-cases ()
      `(;; bool, little and big endian
        (,+bool+           :mock/little-endian-packed-size ((   nil . 1)
                                                            (     t . 1)))
        (,+bool+           :mock/big-endian-packed-size    ((   nil . 1)
                                                            (     t . 1)))
        ;; uint8, little and big endian
        (,+uint8+          :mock/little-endian-packed-size ((     1 . 1)
                                                            (   255 . 1)))
        (,+uint8+          :mock/big-endian-packed-size    ((     1 . 1)
                                                            (   255 . 1)))
        ;; int8, little endian
        (,+int8+           :mock/little-endian-packed-size ((     1 . 1)
                                                            (   127 . 1)
                                                            (    -1 . 1)
                                                            (  -128 . 1)))
        ;; uint16, little and big endian
        (,+uint16+         :mock/little-endian-packed-size ((#x0001 . 2)
                                                            (#x007f . 2)
                                                            (#xff00 . 2)))
        (,+uint16+         :mock/big-endian-packed-size    ((#x0001 . 2)
                                                            (#x007f . 2)
                                                            (#xff00 . 2)))
        ;; float32, little endian
        (,+float32+        :mock/little-endian-packed-size (( 0.0f0 . 4)
                                                            ( 1.0f0 . 4)
                                                            (-2.5f0 . 4)))
        ;; ASCII and UTF-8 strings
        (,+ascii-string+   :mock/little-endian-packed-size ((""     . 1)
                                                            ("a"    . 2)
                                                            ("ab"   . 3)))
        (,+utf-8-string+   :mock/little-endian-packed-size ((""     . 1)
                                                            ("ä"    . 3)
                                                            ("äb"   . 4)))
        ;; Enum
...
We do not verify the emitted code, but its behavior.")
  ...
  (ensure-serialization-cases
      ...
      `(;; bool, little and big endian
        (,+bool+           :mock/little-endian-pack ((   nil . (#x00))
                                                     (     t . (#x01))))
        (,+bool+           :mock/big-endian-pack    ((   nil . (#x00))
                                                     (     t . (#x01))))
        ;; uint8, little and big endian
        (,+uint8+          :mock/little-endian-pack ((     1 . (#x01))
                                                     (   255 . (#xff))))
        (,+uint8+          :mock/big-endian-pack    ((     1 . (#x01))
                                                     (   255 . (#xff))))
        ;; int8, little endian
        (,+int8+           :mock/little-endian-pack ((     1 . (#x01))
                                                     (   127 . (#x7f))
                                                     (    -1 . (#xff))
                                                     (  -128 . (#x80))))
        ;; uint16, little and big endian
        (,+uint16+         :mock/little-endian-pack ((#x0001 . (#x01 #x00))
                                                     (#x007f . (#x7f #x00))
                                                     (#xff00 . (#x00 #xff))))
        (,+uint16+         :mock/big-endian-pack    ((#x0001 . (#x00 #x01))
                                                     (#x007f . (#x00 #x7f))
                                                     (#xff00 . (#xff #x00))))
        ;; float32, little endian
        (,+float32+        :mock/little-endian-pack (( 0.0f0 . (#x00 #x00 #x00 #x00))
                                                     ( 1.0f0 . (#x00 #x00 #x80 #x3f))
                                                     (-2.5f0 . (#x00 #x00 #x20 #xc0))))
        ;; ASCII and UTF-8 strings
        (,+ascii-string+   :mock/little-endian-pack ((""     . (#x00))
                                                     ("a"    . (#x01 #x61))
                                                     ("ab"   . (#x02 #x61 #x62))))
        (,+utf-8-string+   :mock/little-endian-pack ((""     . (#x00))
                                                     ("ä"    . (#x02 #xc3 #xa4))
                                                     ("äb"   . (#x03 #xc3 #xa4 #x62))))
        ;; Enum
...
We do not verify the emitted code, but its behavior.")
  ...
  (ensure-serialization-cases ()
      `(;; bool, little and big endian
        (,+bool+           :mock/little-endian-unpack (((#x00)                . nil)
                                                       ((#x01)                . t)
                                                       ((#x02)                . error)))
        (,+bool+           :mock/big-endian-unpack    (((#x00)                . nil)
                                                       ((#x01)                . t)
                                                       ((#x02)                . error)))
        ;; uint8, little and big endian
        (,+uint8+          :mock/little-endian-unpack (((#x01)                . 1)
                                                       ((#xff)                . 255)))
        (,+uint8+          :mock/big-endian-unpack    (((#x01)                . 1)
                                                       ((#xff)                . 255)))
        ;; int8, little endian
        (,+int8+           :mock/little-endian-unpack (((#x01)                . 1)
                                                       ((#x7f)                . 127)
                                                       ((#xff)                . -1)
                                                       ((#x80)                . -128)))
        ;; uint16, little and big endian
        (,+uint16+         :mock/little-endian-unpack (((#x01 #x00)           . #x0001)
                                                       ((#x7f #x00)           . #x007f)
                                                       ((#x00 #xff)           . #xff00)))
        (,+uint16+         :mock/big-endian-unpack    (((#x00 #x01)           . #x0001)
                                                       ((#x00 #x7f)           . #x007f)
                                                       ((#xff #x00)           . #xff00)))
        ;; float32, little endian
        (,+float32+        :mock/little-endian-unpack (((#x00 #x00 #x00 #x00) .  0.0f0)
                                                       ((#x00 #x00 #x80 #x3f) .  1.0f0)
                                                       ((#x00 #x00 #x20 #xc0) . -2.5f0)))
        ;; ASCII and UTF-8 strings
        (,+ascii-string+   :mock/little-endian-unpack (((#x00)                . "")
                                                       ((#x01 #x61)           . "a")
                                                       ((#x02 #x61 #x62)      . "ab")))
        (,+utf-8-string+   :mock/little-endian-unpack (((#x00)                . "")
                                                       ((#x02 #xc3 #xa4)      . "ä")
                                                       ((#x03 #xc3 #xa4 #x62) . "äb")))
        ;; Enum
